...
<h3>编程思想</h3><br>
面向过程就是分析出解决问题所需要的步骤，然后用函数一步一步实现，使用的时候再一个一个的依次调用<br>
面向对象就是把事物分成一个个对象，然后由对象之间分工合作<br>
面向对象开发思想中，每一个对象都是功能中心，有明确分工<br>
面向对象编程具有灵活，代码可复用，容易维护和开发的有点，更适合多人合作的大型软件项目<br>
面向对象的特性：封装性，继承性，多态性<br>
封装是面向对象思想中重要的一部分，js面向对象可以通过构造函数实现的封装<br>
<h3>原型</h3><br>
构造函数是通过原型分配的函数是所有对象所共享<br>
JavaScript规定，每一个构造函数都有一个prototype属性，指向另一个对象，称为原型对象<br>
这个对象可以挂在函数，对象实例化不会多次创建原型上函数，节约内存<br>
我们可以把那些不变的东西直接定义到prototype对象上，这样所有对象都共享<br>
构造函数何原型对象中的this都指向实例化对象<br>
公共对象写原型对象上面，公共属性写到构造函数<br>
this指向调用者<br>
arr.reverse)反转数组<br>
constructor属性<br>
